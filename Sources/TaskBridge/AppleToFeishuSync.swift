...
class AppleToFeishuSync {
    ...
    private func syncNewRemindersInManagedCalendars(completion: @escaping (Int, [String]) -> Void) {
        ...
        eventKitManager.fetchReminders(in: calendars) { [weak self] reminders in
            ...
            let mappedReminderSignatures = Set(reminders.compactMap { reminder -> String? in
                guard self.dbManager.getMapping(byAppleId: reminder.calendarItemIdentifier) != nil else { return nil }
                return self.duplicateSignature(for: reminder)
            })
            let unmappedReminders = reminders.filter { reminder in
                guard self.dbManager.getMapping(byAppleId: reminder.calendarItemIdentifier) == nil else { return false }
                guard let signature = self.duplicateSignature(for: reminder) else { return true }
                return !mappedReminderSignatures.contains(signature)
            }
    ...
    private func duplicateSignature(for reminder: EKReminder) -> String? {
        guard let calendar = reminder.calendar else { return nil }
        let title = reminder.title.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        let dueTimestamp = eventKitManager.dueInfo(from: reminder)?.timestamp ?? ""
        return "\(calendar.calendarIdentifier)|\(title)|\(reminder.isCompleted)|\(dueTimestamp)"
    }
