...
class FeishuOAuthManager: NSObject {
    ...
    func startLogin(completion: @escaping (Result<String, Error>) -> Void) {
        ...
        let urlString = "https://open.feishu.cn/open-apis/authen/v1/index?redirect_uri=\(encodedRedirectURI)&app_id=\(appId)&state=taskbridge_state"
    ...
    private func handleCallbackPath(_ path: String, connection: NWConnection) {
        ...
        if let code = queryItems.first(where: { $0.name == "code" })?.value {
            ...
            sendHTTPResponse(to: connection, message: "授权成功！您可以关闭此网页并返回 TaskBridge。")
    ...
    private func sendHTTPResponse(to connection: NWConnection, message: String) {
        let html = """
        <html>
        <head><meta charset="utf-8"><title>TaskBridge 授权</title></head>
