...
LarkFlow 是一个 macOS 菜单栏工具，用来在飞书任务和 Apple 提醒事项之间进行双向同步。

如果你平时用飞书协作，但又习惯在 macOS / iPhone / Apple Watch 上通过「提醒事项」查看和处理待办，LarkFlow 可以把两边连接起来：飞书任务会同步到本地提醒事项，本地提醒事项中的变更也会回写到飞书。
...
LarkFlow 会读取飞书任务、清单和分组，并在 Apple 提醒事项中创建对应列表：
...
LarkFlow 需要使用你自己的飞书开放平台应用完成 OAuth 授权。
...
LarkFlow 会在本机临时启动一个 HTTP 回调服务，用于接收飞书 OAuth 授权结果。
...
1. 启动 LarkFlow。
2. 在菜单栏点击 LarkFlow 图标。
3. 点击「偏好设置...」，填写飞书 App ID 和 App Secret。
4. 点击「授权访问」，授予 Apple 提醒事项权限。
5. 点击「飞书授权登录」，在浏览器中完成飞书授权。
6. 回到 LarkFlow，点击「立即同步」开始同步。

授权完成后，LarkFlow 会在后台自动同步。

## 本地数据保存位置

LarkFlow 会在本机保存同步映射和授权信息：

- 同步数据库：`~/Library/Application Support/com.namrood.LarkFlow/larkflow.sqlite`
...
- LarkFlow 需要 Apple 提醒事项完全访问权限，用于创建、更新和删除提醒事项。
- LarkFlow 使用你的飞书 OAuth 授权访问任务数据。
- App ID、App Secret 和 Token 当前保存在本机 `UserDefaults` 中。
- 建议不要使用他人提供的未知构建版本，请尽量自行从源码构建。
- 如果你不再使用 LarkFlow，可以在飞书后台取消应用授权，并删除本地数据库。
